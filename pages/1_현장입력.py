...
# --- 💡 [필수] 공수 계산 헬퍼 함수 (삭제 금지) --- [cite: 2026-03-05]
...
# 1️⃣ 작업 현장 선택
st.write("### 🚩 작업 현장 선택")
selected_place = st.segmented_control("현장을 선택하세요.", options=workplace_list, default="A동", key="workplace_selector")
        ...
        for main, subs in task_hierarchy.items():
            if subs:
                ...
                if st.button(f"{'▼' if is_ex else '▶'} {main}", key=f"m_{main}", use_container_width=True):
                    st.session_state.expanded_main = main if not is_ex else None; st.rerun()
                if is_ex:
                    for sub in subs:
        ...
        for idx, task in enumerate(tasks):
            with cols[idx % 4]:
                with st.container(border=True):
                    ...
                    # 💡 [신규] 상태에 따른 입력창 전환 로직 [cite: 2026-03-10]
                    ...
                    if task['status'] == "running":
                        ...
                        # 정지 중: 완료 수량 입력 [cite: 2026-03-05]
                        done_q = st.number_input("📦 완료 수량 입력", min_value=0, value=int(task.get('completed_quantity', 0)), key=f"q_{task['id']}")
                        if st.button("실적 저장", key=f"up_q_{task['id']}", use_container_width=True):
                    ...
                    if c2.button("🏁 종료", key=f"e_{task['id']}", type="primary", use_container_width=True):
                        ...
                        for entry in final_h:
                            ...
                            supabase.table("work_logs").insert({
                                "work_date": entry['date'], "task": task['task_type'], "workers": task['workers'],
                                "quantity": round(task.get('completed_quantity', 0) * weight),
                                "duration": round(entry['man_seconds'] / 3600, 2), "plan_id": task.get('plan_id'),
                                "memo": f"현장: {place} / {task['session_name']}"
                            }).execute()
                        supabase.table("active_tasks").delete().eq("id", task['id']).execute(); st.balloons(); st.rerun()
