...
# 쿠키 매니저 초기화 (로그인 유지를 위한 도구)
cookie_manager = stx.CookieManager()

# 세션 상태 복구 로직 (새로고침 시 쿠키에서 정보 호출)
...
if st.session_state.role is None:
    ...
    # 페이지 정의
    admin_dash = st.Page(show_admin_dashboard, title="통합 대시보드", icon="📊")
    staff_input = st.Page("pages/1_현장입력.py", title="현장기록", icon="📝")

    # 권한별 메뉴 구성 (Admin은 모두, Staff는 기록 페이지만)
