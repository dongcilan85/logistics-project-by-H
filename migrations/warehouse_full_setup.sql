-- =========================================================
-- ⚠️⚠️⚠️ 경고: 이 파일은 운영 환경에서 실행하지 마세요 ⚠️⚠️⚠️
-- ---------------------------------------------------------
-- 이 스크립트는 warehouse_inventory / warehouse_history 테이블을
-- DROP CASCADE 로 완전히 삭제하고 더미 데이터로 재생성합니다.
-- 실수로 실행 시 운영 재고/이력 데이터가 영구 손실됩니다.
--
-- ▸ 현재 운영 스키마는 ../db_master.sql 에 멱등성 보장되어 관리됩니다.
-- ▸ 이 파일은 최초 셋업 이력 보존용으로만 보관되며, 신규 환경에서도
--   db_master.sql 사용을 권장합니다.
-- ▸ 정말로 초기화해야 한다면 백업 후 수동으로 한 줄씩 검토 실행하세요.
-- =========================================================
-- IWP 창고관리 시스템 (Phase 7) 기반 DB 스키마 셋업 (초기 1회용)
-- =========================================================
